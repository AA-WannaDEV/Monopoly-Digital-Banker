...
function renderSplash() {
  ...
  app.innerHTML = `
    <div class="splash-screen fade-in">
      <button class="theme-toggle-btn" id="splash-theme-toggle" style="position: absolute; top: 20px; right: 20px;">
  ...
  document.getElementById('splash-theme-toggle').addEventListener('click', (e) => {
    toggleTheme();
    e.target.innerHTML = document.documentElement.getAttribute('data-theme') === 'dark' ? '☀️' : '🌙';
  });
}
    ...
    <div class="dashboard">
      ...
      <header class="dash-header">
        ...
        <div class="dash-header-right">
          <button class="theme-toggle-btn" id="dash-theme-toggle" title="Toggle Theme">
            ${document.documentElement.getAttribute('data-theme') === 'dark' ? '☀️' : '🌙'}
          </button>
...
function renderPlayerCard(player, isActive) {
  ...
  const oldBalance = previousBalances[player.id];
  const balanceChanged = oldBalance !== undefined && oldBalance !== player.balance;
  const animClass = balanceChanged ? 'balance-animating' : '';
  
  return `
    <div class="player-card fade-in ${isActive ? 'player-card-active' : ''}" 
         style="--player-color: ${player.colorRing}"
         data-player-id="${player.id}" title="Click to view details">
      ...
      <div class="player-balance-display ${animClass}">
  ...
  document.querySelectorAll('.player-card').forEach(card => {
    card.addEventListener('click', () => {
      if (card.dataset.playerId) {
        showPlayerInfoDrawer(card.dataset.playerId);
      }
    });
  });
...
function showTradeModal(player) {
  ...
  const renderStep1 = () => `
    <div class="trade-modal fade-in" id="trade-step-1">
      <p class="modal-instruction">Step 1: Select a trade partner</p>
      <div class="trade-partner-list">
        ${otherPlayers.map(p => `
          <button class="trade-partner-btn property-select-btn" data-partner-id="${p.id}" style="justify-content: flex-start; gap: 12px;">
            <span class="partner-token" style="font-size: 24px;">${TOKENS.find(t => t.id === p.token)?.emoji}</span>
            <span class="partner-name prop-select-name">${p.name}</span>
            <span class="partner-balance prop-select-price">$${p.balance.toLocaleString()}</span>
          </button>
        `).join('')}
      </div>
    </div>
  ...
  const renderStep2 = (player1, player2) => {
    ...
    return `
      <div class="trade-builder fade-in">
        <div class="trade-columns">
          <div class="trade-column">
            <h3 class="trade-col-title">${TOKENS.find(t => t.id === player1.token)?.emoji} ${player1.name} Offers</h3>
            <div class="trade-cash-input">
              <label>Cash:</label>
              <div class="balance-input-group" style="padding: 4px;">
                <span class="currency-symbol">$</span>
                <input type="number" id="trade-cash-1" value="0" min="0" max="${player1.balance}" style="width: 100%; border: none;" />
              </div>
            </div>
            <div class="trade-props" id="trade-props-1">
              ...
              `).join('')}
              ${p1Props.length === 0 ? '<p class="trade-empty" style="color: #999;">No properties</p>' : ''}
            </div>
            ${p1Goojf > 0 ? `
              <label class="trade-prop-check">
          ...
          <div class="trade-column">
            ...
            <div class="trade-cash-input">
              ...
              <div class="balance-input-group" style="padding: 4px;">
                <span class="currency-symbol">$</span>
                <input type="number" id="trade-cash-2" value="0" min="0" max="${player2.balance}" style="width: 100%; border: none;" />
              </div>
            </div>
            <div class="trade-props" id="trade-props-2">
              ...
              `).join('')}
              ${p2Props.length === 0 ? '<p class="trade-empty" style="color: #999;">No properties</p>' : ''}
            </div>
            ${p2Goojf > 0 ? `
              <label class="trade-prop-check">
  ...
  const content = `<div id="trade-container">${renderStep1()}</div>`;
  const closeModal = showModal('🤝 Trade', content);

  const bindStep1 = () => {
    document.querySelectorAll('.trade-partner-btn').forEach(btn => {
      btn.addEventListener('click', () => {
        const partner = state.players.find(p => p.id === btn.dataset.partnerId);
        document.getElementById('trade-container').innerHTML = renderStep2(player, partner);
        bindStep2(partner);
      });
    });
  };

  const bindStep2 = (player2) => {
...
function showPlayerInfoDrawer(playerId) {
  const player = state.players.find(p => p.id === playerId);
  if (!player) return;
  ...
  let propertiesHtml = '';
  if (player.ownedPropertyIds.length === 0) {
    propertiesHtml = '<p style="color: #888; font-style: italic;">No properties owned.</p>';
  } else {
    // Group properties by color
    const groups = {};
    player.ownedPropertyIds.forEach(id => {
      const p = PROPERTIES.find(pr => pr.id === id);
      if (!groups[p.colorGroup]) groups[p.colorGroup] = { group: COLOR_GROUPS[p.colorGroup], items: [] };
      groups[p.colorGroup].items.push({ p, state: state.propertyStates[id] });
    });
    
    propertiesHtml = Object.values(groups).map(g => `
      <div style="margin-bottom: 12px;">
        <div style="display: flex; align-items: center; margin-bottom: 4px;">
          <span style="display: inline-block; width: 12px; height: 12px; background: ${g.group?.hex || '#999'}; border-radius: 2px; margin-right: 8px;"></span>
          <strong>${g.group?.name || 'Other'}</strong>
        </div>
        ${g.items.map(item => `
          <div class="property-row" style="padding: 4px 8px;" onclick="document.getElementById('player-drawer-close').click(); setTimeout(() => __dispatch({type: 'NONE'}), 100); setTimeout(() => document.querySelector('.property-row[data-property-id=\\'${item.p.id}\\']').click(), 200)">
            <span style="flex: 1">${item.p.name}</span>
            <span style="font-size: 11px;">
              ${item.state.isMortgaged ? '<span style="color:#e65100;">(M)</span>' : ''}
              ${item.state.hasHotel ? '🏨' : item.state.houses > 0 ? `🏠x${item.state.houses}` : ''}
            </span>
          </div>
        `).join('')}
      </div>
  ...
  const cardHistory = player.cardHistory || [];
  let historyHtml = cardHistory.length === 0 
    ? '<p style="color: #888; font-style: italic;">No cards drawn yet.</p>'
    : cardHistory.slice().reverse().map(c => `
        <div class="card-history-item">
          <span class="card-history-deck" style="color: ${c.deck === 'chance' ? '#F7941D' : '#4A90E2'}">${c.deck === 'chance' ? '❓' : '📦'}</span>
          <span class="card-history-text">${c.text}</span>
          <span class="card-history-turn">T${c.turn}</span>
        </div>
      `).join('');
  ...
  const content = `
    <div class="player-drawer-overlay visible" id="player-drawer-overlay">
      <div class="player-drawer">
        <div class="drawer-header" style="border-bottom: 4px solid ${player.colorRing}">
          <div style="display: flex; align-items: center; gap: 12px;">
            <span style="font-size: 32px;">${token?.emoji}</span>
            <div>
              <h2 style="margin: 0; font-family: var(--f-display);">${player.name}</h2>
              <span style="font-size: 12px; opacity: 0.8;">${player.isBankrupt ? 'BANKRUPT' : player.isInJail ? 'IN JAIL' : 'Active Player'}</span>
            </div>
          </div>
          <button class="drawer-close" id="player-drawer-close">✕</button>
        </div>
        ...
        <div class="drawer-content">
          <div class="drawer-section">
            ...
            <div style="display: flex; justify-content: space-between; margin-bottom: 8px;">
              ...
              <strong style="color: var(--c-success); font-family: monospace; font-size: 18px;">$${player.balance.toLocaleString()}</strong>
            </div>
            <div style="display: flex; justify-content: space-between;">
              <span>Net Worth:</span>
              <strong style="font-family: monospace; font-size: 18px;">$${netWorth.toLocaleString()}</strong>
            </div>
          </div>
          
          <div class="drawer-section">
            <h3>Properties (${player.ownedPropertyIds.length})</h3>
            ${propertiesHtml}
          </div>
          ...
          <div class="drawer-section">
            ...
            ${goojfCount > 0 ? `
              <div style="display: flex; gap: 8px; align-items: center;">
                ...
                <span>Get Out of Jail Free: <strong>${goojfCount}</strong></span>
              </div>
            ` : '<p style="color: #888; font-style: italic;">No special items.</p>'}
          </div>
  ...
  if (container) {
    ...
    document.getElementById('player-drawer-overlay').addEventListener('click', (e) => {
      if (e.target.id === 'player-drawer-overlay') closeDrawer();
    });
  }
}
